...
test("proxy hls route is extractor-aware for DLHD/Vavoo HTML pages", async () => {
  const fs = await import("node:fs/promises");
  const source = await fs.readFile(new URL("../src/router.ts", import.meta.url), "utf8");
  assert.match(source, /non-manifest page/);
  assert.match(source, /extractVideo\(target\.toString\(\)/);
  assert.match(source, /daddylive\|dlhd\|dlive\|vavoo/);
});
